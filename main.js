...
const IMAGE_MAP = {
  ...
  "Blue Slush": "alani-blueslush.webp",
  "Sherbet Swirl": "alani-sherbetswirl.webp",
  "Cotton Candy": "alani-cottoncandy.webp",
  ...
  "Pringles": "pringles.webp",
  "Cheezits": "cheez-it.webp", 
  "Rice Krispie Treat": "rice-krispies.webp",
...
// Auto-generate item list from IMAGE_MAP except DEFAULT
const ALL_SNACK_ITEMS = Object.keys(IMAGE_MAP).filter(k => k !== "DEFAULT");
...
function renderItemsGrid(order) {
  ...
  if (order.itemsCompleted[idx]) return;

  // Mark as completed once (one-way)
  order.itemsCompleted[idx] = true;

  // Count toward inventory
...
teacherButton.addEventListener("click", () => {
  requireTeacherPin(() => {
    const wasHidden = teacherPanel.classList.contains("hidden");
...
function renderNewOrderItemsGrid(selection) {
  const grid = document.getElementById("newOrderItemsGrid");
  grid.innerHTML = "";

  ALL_SNACK_ITEMS.forEach(itemName => {
    const tile = document.createElement("div");
    tile.className = "item-tile" + (selection[itemName] ? " selected" : "");

    const img = document.createElement("img");
    img.className = "item-image";
    img.src = "images/" + (IMAGE_MAP[itemName] || "default.webp");
    img.alt = itemName;
    ...
    tile.addEventListener("click", () => {
      // One-way selection just like student view
      if (!selection[itemName]) {
        selection[itemName] = true;
        renderNewOrderItemsGrid(selection);
      }
    });
...
function setupNewOrderHandlers() {
  ...
  const saveButton = document.getElementById("saveNewOrderButton");
  ...
  newOrderButton.addEventListener("click", () => {
    ...
    renderNewOrderItemsGrid(selection);
    newOrderPanel.classList.toggle("hidden");
  });
  ...
  saveButton.addEventListener("click", () => {
    ...
    const selectedItems = Object.keys(selection).filter(k => selection[k]);
    ...
    const newOrder = {
      staffName: name,
      items: selectedItems,
      itemsCompleted: selectedItems.map(() => false),
      isCompleted: false
    };
    ...
    newOrderPanel.classList.add("hidden");
    alert("Order added!");
  });
}

// CSV parser that handles quotes, commas inside quotes, etc.
...
// Requires ONLY a Name/Staff/Student column.
// Handles quoted cells such as "Chips Ahoy, Oreos" correctly.
function parseCsvToOrders(text) {
  const rows = parseCsvWithQuotes(text);
  if (!rows.length) return [];

  // ---- Detect header row + name column ----
  let headerRowIndex = null;
  let nameColIndex = null;

  for (let i = 0; i < rows.length; i++) {
    const lower = rows[i].map(c => c.toLowerCase());
    nameColIndex = lower.findIndex(c =>
      c.includes("name") || c.includes("staff") || c.includes("student")
    );
    if (nameColIndex !== -1) {
      headerRowIndex = i;
      break;
  ...
  if (headerRowIndex === null) {
    alert("CSV must contain a header with a Name/Staff/Student column.");
    return [];
  }
  ...
  for (let i = headerRowIndex + 1; i < rows.length; i++) {
    ...
    const staffName = (cols[nameColIndex] || "").trim();
    if (!staffName) continue;

    // Only take columns AFTER the name column — ignore anything before it
const itemCells = cols.slice(nameColIndex + 1);


 const items = itemCells.flatMap(rawCell => {
  const cell = rawCell.trim();

  // CASE 1: semicolon-separated items
  if (cell.includes(";")) {
    return cell
      .split(";")
      .map(i => i.trim())
  ...
  // CASE 2: originally quoted? (multiple items in one quoted cell)
  const wasQuoted = rawCell.startsWith('"') && rawCell.endsWith('"');
  ...
  // CASE 3: DEFAULT — a normal single item (commas inside text allowed)
  return [cell];
})
// ⭐ FIX: remove ALL empty/blank items
.filter(i => i && i.trim().length > 0);
    ...
    items.forEach(item => {
      if (!grouped[staffName].includes(item)) {
